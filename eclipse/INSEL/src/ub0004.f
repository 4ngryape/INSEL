...
C       #BP(9)   Input node heat source 1            [-]
C       #BP(10)   Output node heat source 1            [-]
C       #BP(11)   Input node heat source 1            [-]
C       #BP(12)   Output node heat source 1            [-]
...
C           Initial heat content
            OUT(IP(12)+1) = BP(1) * BP(4) * BP(5) * BP(8)
         ELSE
C           Destructor call
         END IF
         RETURN
...
C           Restrict time step to about 360 seconds maximum
         ...
         DO I = 1,MTS
           ...
           QS1 = IN(MC(IS)) * BP(CP)* (IN(TCI(IS)) - TNODET(IP(12)))*TS
      ...
      IF ((IS.EQ.1.AND.bp(9).EQ.0).OR.(is.EQ.2.AND.bp(11).EQ.0)) THEN
      ...
      IF ((IS.EQ.1.AND.bp(9).GT.0).OR.(is.EQ.2.AND.bp(11).GT.0)) THEN
...
C     Festgelegte Wahl der Schicht in die eingespeist wird
      IF ((IS.EQ.1.AND.bp(9).GT.0).OR.(is.EQ.2.AND.bp(11).GT.0)) THEN
      IF (INCOL .NE. K .AND. INLOAD .NE. K .AND. K .LE. OUTCOL) THEN
C                    Layer without direct input
                  ...
                  END IF
      IF (INCOL .NE. K .AND. INLOAD .NE. K .AND. K .GT. OUTCOL) THEN
C                    Layer without direct input
                     IF (INLOAD .GT. K) MDOTL = IN(ML(IS))
                     DELTAM = MDOTC - MDOTL
                     IF (DELTAM .GT. 0.0) DELTADOWN = 1
...
C     Automatische Wahl der Schicht in die eingespeist wird
      IF ((IS.EQ.1.AND.bp(9).EQ.0).OR.(is.EQ.2.AND.bp(11).EQ.0)) THEN
...
C                    Layer with collector and load input
        ...
        IF (K .GT. 1 .AND. K .LT. IP(12).AND. K .NE. BP(9)) THEN
