...
C     #IN(5) Global radiation in the window plane [W/m$^2$]
C     #IN(6) Diffuse radiation in the window plane [W/m$^2$]
C     #IN(7) Ambient temperature / \degC
C     #IN(8) Sky temperature / \degC
C     #IN(9) Wind speed / m\,s$^{-1}$
C     #IN(10) Internal temperature / \degC
C     #IN(11) Incidence angle of beam radiation [degrees]
C     #in(19) Any other input 1
C     #IN(13) Any other input 2
C     #IN(14) Any other input 3
C  #Outputs
C     #OUT(1) Heat flux through the window [W/m$^2$]
C     #OUT(2) Beam radiation flux through the window [W/m$^2$]
C     #OUT(3) Diffuse radiation flux through the window [W/m$^2$]
C     #OUT(4) Internal surface temperature [\degC]
C     #OUT(5) External surface temperature [\degC]
C     #OUT(6) Conv flux [W/m$^2$]
C     #OUT(7) Any other output 1
C     #OUT(8) Any other output 2
C     #OUT(9) Any other output 3
C  #Parameters
C     #BP(1) h_ci (W/m2)
C     #BP(2) h_ri (W/m2)
C     #BP(3) T_i (°c)
...
C     #BP(7) g-value
C     #BP(8) N_l
C     #BP(9) e /m + U /Wm-1K-1 + ro /kgm-3 + Cp /Jkg-1K-1
C  #Strings
C     #None
C #Internals
C  #Integers
...
CCCCC Eventuell als BP herausfuehren....................................
      ...
      REAL           QLEFT,GSOLAR,HCO,TA,HRO,TSKY,DT,QCOND,TLAYER2
     &                  ,ELOSS, CONVI, CONVO
      REAL     HRI
...
C      END IF
      ...
      HCO  = BP(4)
      HRO  = BP(5)
      if(exterior) HCO = HCO + 3.68*IN(9)         !http://editorial.cda.ulpgc.es/ftp/ambiente/antesol/TESIS/Cap4.pdf
...
C        Absolute time is memorised in rp(1), unit is "seconds"
         TIME  = IN(1)
         RP(1) = TIME
C        Determine U-value from thickness bp(9+..) divided by
C        conductivity bp(9+nlay+..), U value calculated from interior
C        surface temperature, i.e. without 1/hci
         SUMSL = 1.0 / HCI
         DO I = 1,NLAYi
            THICK(I)  = BP(9+4*(I-1))
            LAMBDA(I) = BP(10+4*(I-1))
...
C        With deltax=thick/ndiv follows: ndiv<thick*sqrt(0.5/(a*deltat))
         ...
         DO I = 2,NLAYi
            if(TAU(icont).le.DT) then
                ...
                RHO_a=RHO(icont)
                CAP_a=CAP(icont)
                THICK(icont)=BP(9+4*(I-1))+THICK_a
                LAMBDA(icont)=THICK(icont)/(BP(9+4*(I-1))/BP(10+4*(I-1))
     &             +THICK_a/LAMBDA_a)
                RHO(icont)=(BP(11+4*(I-1))*BP(9+4*(I-1))+
     &            RHO_a*THICK_a)/THICK(icont)
                XX=RHO_a*THICK_a/(RHO_a*THICK_a+
     &             BP(11+4*(I-1))*BP(9+4*(I-1)))
                CAP(icont)=BP(12+4*(I-1))*(1-XX)+CAP_a*XX
C           Thermal diffusivity
                A(icont)  = LAMBDA(icont) / (RHO(icont) * CAP(icont))
                TAU(icont) = THICK(icont)**2/A(icont)
            else
                icont=icont+1
                THICK(icont)  = BP(9+4*(I-1))
                LAMBDA(icont) = BP(10+4*(I-1))
                RHO(icont)    = BP(11+4*(I-1))
                CAP(icont)    = BP(12+4*(I-1))
C               Thermal diffusivity
                A(icont)  = LAMBDA(icont) / (RHO(icont) * CAP(icont))
                TAU(icont) = THICK(icont)**2/A(icont)
            end if
...
C        time=IN(1)
        ...
        BB = HCO + HRO
        if (EXTERIOR) then
          TEXT = (HCO*IN(7) + HRO*IN(8)) / BB
          TEMP(1,1) = ((1/BB+1/C2L)*IN(10)+(1/AA)*TEXT
     &        + 1/AA*(1/BB+1/C2L)*(IN(2)+IN(3)+IN(4))
     &        + 1/AA/BB*(IN(5)-IN(6)))
     &        / (1/AA + 1/C2L + 1/BB)
          TEMP(NLAY+1,1) = ((1/AA+1/C2L)*TEXT+(1/BB)*IN(10)
     &        + 1/BB*(1/AA+1/C2L)*(IN(5)-IN(6))
     &        + 1/AA/BB*(IN(2)+IN(3)+IN(4)))
     &        / (1/AA + 1/C2L + 1/BB)
          CONVO = HCO * (IN(7)-TEMP(NLAY+1,1))
        else
          TEMP(1,1) = ((1/BB+1/C2L)*IN(10)+(1/AA)*IN(11)
     &        + 1/AA*(1/BB+1/C2L)*(IN(2)+IN(4)+IN(5))
     &        + 1/AA/BB*(IN(6)+IN(8)+IN(9)))
     &        / (1/AA + 1/C2L + 1/BB)
          TEMP(NLAY+1,1) = ((1/AA+1/C2L)*IN(11)+(1/BB)*IN(10)
     &        + 1/BB*(1/AA+1/C2L)*(IN(6)+IN(8)+IN(9))
     &        + 1/AA/BB*(IN(2)+IN(4)+IN(5)))
     &        / (1/AA + 1/C2L + 1/BB)
          CONVO = HCO * (IN(11)-TEMP(NLAY+1,1))
        end if
        CONVI = HCI * (IN(10)-TEMP(1,1))
        OUT(1)=TEMP(1,1)
        OUT(2)=TEMP(NLAY+1,1)
        OUT(3)=CONVI
        OUT(4)=CONVO
...
C        surface is called RP(4+nlay+1) and is the old temperature
     ...
     &          * (C2L * TLAYER2 + IN(2) + HCI*IN(10) + IN(3) + IN(4)
     &          + HRI * TOLD(1,1)))
...
C        external solar irradiance IN(5)-IN(6), convective gains IN(7), radiative
C        heat gains in(8), radiative gains from heat source IN(9)
         QLEFT = C2R * (TOLD(IRIGHT,JRIGHT) - TOLD(NLAY+1,1))
         IF (EXTERIOR) THEN
            TSKY    = IN(8)
C           Outside heat transfer coefficient
            !HCOWIND = 3.575 * IN(10) !to be changed
            !HCOFREE = 1.78 * ABS(TOLD(NLAY+1,1) - IN(8))**(1.0/3.0)
            !HCO     = 9.42 + 3.68*IN(9)         !http://editorial.cda.ulpgc.es/ftp/ambiente/antesol/TESIS/Cap4.pdf
            GSOLAR  = 0      !IN(5)-IN(6)
            TA      = IN(7)
     ...
     &             * (QLEFT + IN(6) + HCO*IN(11) + IN(8) + IN(9)
     &             + HRO*TOLD(NLAY+1,1)))
     &             / (1 + DT / C1R * (HRO + HCO))
            ELOSS = 0.0
            CONVO = HCO*(IN(11)-TEMP(NLAY+1,1))
...
C        Store new temperature field in RP field
         ...
         END DO
      END DO
      OUT(1) = (U-1.0 / HCI-1.0 / HCO)*(TEMP(NLAY+1,1)-TEMP(1,1))
      OUT(2) = (IN(5) - IN(6)) * BP(7)
      OUT(3) = IN(6) * BP(7)
      OUT(4) = TEMP(1,1)
      OUT(5) = TEMP(NLAY+1,1)
