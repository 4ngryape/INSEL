...
C     temperature field
      x = IN(10)/(RHO(1)*CAP(1)*THICK(1))*10
      if(SS) then
        C2L = RP(195)
        AA = HCI + HRI
        BB = HCO + HRO
        if (EXTERIOR) then
          ...
          TEMP(1,1) = x + ((1/BB+1/C2L)*IN(9)+(1/AA)*TEXT
     ...
     &        / (1/AA + 1/C2L + 1/BB)
          ...
          TEMP(1,1) = x + ((1/BB+1/C2L)*IN(8)+(1/AA)*IN(9)
     &        + 1/AA*(1/BB+1/C2L)*(IN(2)+IN(3)+IN(4))
